...
# Provider model catalogs — capability, ordering and pricing — live in their
# own module: they are reference data about other people's model line-ups,
# edited when a provider ships or retires a model rather than when this
# module's analysis logic changes. Only the names this module actually uses
# are imported; the reference tables themselves are read straight from
# blink_downloader.model_catalog by anything that needs them.
...
class BaseAnalyzer(abc.ABC):
    ...
    def _is_night(clip_timestamp: str) -> bool:
        ...
        :meth:`_time_of_day_segment`, and the same hour boundaries, so the
