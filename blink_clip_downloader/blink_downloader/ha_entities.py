...
def _marked_asset_names(result: Any) -> list[str]:
    """The assets marked on the Assets tab that *result*'s security events
    were about, by the names the user gave them, once each and in the order
    they came up — so an automation can act on "something happened at the
    Mailbox". The protected vehicle is left out: it has no name of its own,
    only its description, and every vehicle event already says its camera.
    """
    names: list[str] = []
    for event in getattr(result, "security_events", None) or []:
        name = str(getattr(event, "asset_name", "") or "")
        if name and getattr(event, "asset_type", "") != "vehicle" and name not in names:
            names.append(name)
    return names
...
def clip_analyzed_event_data(
    ...
    return {
        ...
        "assets": _marked_asset_names(result),
