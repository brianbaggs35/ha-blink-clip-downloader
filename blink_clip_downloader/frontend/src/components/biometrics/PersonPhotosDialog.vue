...
import { initials, isLegacyPhoto, photoProblems, type Person } from './people'
...
const photos = computed(() =>
  (props.person?.photos ?? []).map((photo) => ({ photo, problems: photoProblems(photo, props.frameWidth) })),
)
</script>
...
<template>
  ...
  >
    ...
    <div class="photo-grid">
      <figure
        v-for="{ photo, problems } in photos"
        :key="photo.id"
        class="photo-item"
