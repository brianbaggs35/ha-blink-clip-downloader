...
const PROVIDERS: ProviderInfo[] = [
  {
    ...
    configFields: [
      ...
      'ollama_model — vision model to use (e.g. llava:7b, llama3.2-vision:11b)',
    ],
    notes: [
  ...
  },
  {
    ...
    configFields: [
      ...
      'anthropic_model — Claude model to use, defaults to claude-haiku-4-5',
    ],
    notes: [
  ...
  },
  {
    ...
    configFields: [
      ...
      'openai_model — GPT model to use, defaults to gpt-4o-mini',
    ],
    notes: [
...
<template>
  <div class="auto-content">
    ...
    <p>
      Six AI providers are supported for clip analysis — set <code>ai_provider</code> in the add-on's Configuration tab
      to one of the values below. Use <strong>Fetch Models</strong> on the AI tab to browse live model availability and
    ...
    <p style="font-size: 0.85rem; color: var(--muted); line-height: 1.55; max-width: 70ch">
      ...
      When <code>ai_escalation_provider</code> is set, every clip is first analyzed with the normal
      <code>ai_provider</code>/model (tier 1). Only clips tier 1 flags as suspicious are re-analyzed by the escalation
      provider (tier 2) for a closer second opinion, and the tier-2 verdict — not tier 1's — is what gets recorded and
      alerted on. Tier 2 can be any provider, including a different one than tier 1 — e.g. a fast OpenAI model
