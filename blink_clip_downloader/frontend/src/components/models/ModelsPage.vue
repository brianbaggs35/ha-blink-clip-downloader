...
const PROVIDERS: ProviderInfo[] = [
  {
    ...
    tagline:
      'Sends frames to an Ollama server running a vision model on your local network. Free and fully private — no data leaves your home.',
    configFields: [
      'Ollama Server URL — URL of your Ollama server, e.g. http://192.168.1.10:11434',
      'Ollama Vision Model — vision model to use (e.g. llava:7b, llama3.2-vision:11b)',
    ],
    notes: [
  ...
  },
  {
    ...
    tagline:
      'Sends frames to the Moondream Cloud API. Billed per request; reasoning mode is enabled by default for better spatial analysis.',
    configFields: ['Moondream API Key — API key from moondream.ai'],
  ...
  },
  {
    ...
    configFields: [
      ...
      'Anthropic Model — Claude model to use, defaults to claude-haiku-4-5',
    ],
    notes: [
  ...
  },
  {
    ...
    name: 'OpenAI (GPT)',
    tagline:
      'Uses the OpenAI API for vision analysis. Billed per token. Any OpenAI vision model is supported (GPT-4o, GPT-4.1, GPT-4-Turbo, GPT-5, and variants).',
    configFields: [
      'OpenAI API Key — API key from platform.openai.com',
      'OpenAI Model — GPT model to use, defaults to gpt-4o-mini',
    ],
    notes: [
...
<template>
  <div class="auto-content">
    ...
    <p>
      Six AI providers are supported for clip analysis — set the AI Provider option in the add-on's Configuration tab to
      one of the values below. Use <strong>Fetch Models</strong> on the AI tab to browse live model availability and
      pricing for the provider you've configured; this page is a reference for choosing between them.
    </p>
    ...
    <p style="font-size: 0.85rem; color: var(--muted); line-height: 1.55; max-width: 70ch">
      Most motion clips are not suspicious, so paying for a strong model on every clip is wasted spend or wasted time.
      When the AI Escalation Provider option is set, every clip is first analyzed with the normally configured AI
      Provider/model (tier 1). Only clips tier 1 flags as suspicious are re-analyzed by the escalation provider (tier 2)
      for a closer second opinion, and the tier-2 verdict — not tier 1's — is what gets recorded and alerted on. Tier 2
      can be any provider, including a different one than tier 1 — e.g. a fast OpenAI model escalating to Moondream
      Cloud or Claude for a closer second look. Track tier-1 vs. escalation token usage and cost separately on the
      <strong>AI Usage</strong> tab.
    </p>
