...
describe('DashboardBuilderCard', () => {
  ...
  it('drops the status card when switched off', async () => {
    ...
    expect(wrapper.findAllComponents({ name: 'CodeBlock' })[1].props('code')).not.toContain(
      'type: entities',
    )
  })
  ...
  it('threads the chosen display into the cast script', async () => {
    ...
    expect(wrapper.findAllComponents({ name: 'CodeBlock' })[2].props('code')).toContain(
  ...
  it('still renders with no cameras at all', () => {
    expect(mountCard([]).findAllComponents({ name: 'CodeBlock' })[0].props('code')).toContain(
      '<no cameras found>',
    )
  })
