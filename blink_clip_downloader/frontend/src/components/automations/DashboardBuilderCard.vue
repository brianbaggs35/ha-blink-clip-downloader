...
<template>
  <div class="dash-builder">
    ...
    <template v-else>
      ...
      <p class="dash-note">
        The card embeds <code>{{ embedUrl }}</code> — this tab's Security Feed with the navigation hidden. It follows
...
/* See RecipeFieldInput.vue — an InputNumber's inner input does not shrink
   on its own, and overlapped the next column here. */
.dash-wide :deep(.p-inputnumber-input) {
  width: 100%;
  min-width: 0;
}
.dash-help {
