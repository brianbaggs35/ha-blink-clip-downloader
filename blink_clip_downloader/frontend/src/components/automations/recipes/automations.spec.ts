...
describe('cast the feed', () => {
  it('casts the chosen view and stops again', () => {
    const parsed = load(build('cast-security-feed', { media_player: 'media_player.hub', stop_after: 2 })) as {
      actions: { action: string; data?: Record<string, string>; delay?: string }[]
    }
...
describe('announce a clip', () => {
  ...
  it('speaks through the chosen tts entity onto the chosen speaker', () => {
    const parsed = load(build('announce-clip', { tts_entity: 'tts.piper', media_player: 'media_player.office' })) as {
      actions: { action: string; target: { entity_id: string }; data: Record<string, string> }[]
    }
...
describe('new clip notification', () => {
  ...
  it('filters by source when sources are chosen', () => {
    expect(build('new-clip-notify', { sources: ['pir', 'liveview'] })).toContain('["pir", "liveview"]')
  })
})

describe('long clip', () => {
  it('compares against the chosen duration', () => {
    expect(build('long-motion-clip', { seconds: 45 })).toContain('(trigger.event.data.duration | float(0)) > 45')
  })
})
