...
describe('urls', () => {
  ...
  it('tolerates a trailing slash and an empty url', () => {
    expect(snapshotUrl('http://ha.local:8099///', 'A')).toBe(
      'http://ha.local:8099/api/security-feed/snapshot/A',
    )
...
describe('dashboard yaml', () => {
  it('is a valid lovelace view with one picture-entity per camera', () => {
    ...
    expect(view.cards[0].cards?.map((c) => c.entity)).toEqual([
  ...
  it('drops the storage and status cards when they are switched off', () => {
    ...
    const bare = load(
      dashboardYaml(options({ includeStorage: false, includeStatus: false })),
    ) as { views: { cards: unknown[] }[] }
