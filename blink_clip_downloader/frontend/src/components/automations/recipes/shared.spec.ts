...
describe('conditions', () => {
  ...
  it('eventNumberCondition is omitted at zero, since zero accepts everything', () => {
    ...
    expect(eventNumberCondition('confidence', 0.6)).toContain(
      '(trigger.event.data.confidence | float(0)) >= 0.6',
    )
  })

  it('timeWindowCondition renders whichever end is set', () => {
    expect(timeWindowCondition('', '')).toBe('')
    expect(timeWindowCondition('07:00:00', '')).toBe(
      ['  - condition: time', '    after: "07:00:00"'].join('\n'),
    )
    expect(timeWindowCondition('', '22:00:00')).toBe(
      ['  - condition: time', '    before: "22:00:00"'].join('\n'),
    )
