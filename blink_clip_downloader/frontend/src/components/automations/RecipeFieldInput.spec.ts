...
describe('RecipeFieldInput', () => {
  ...
  it('shows help text only when the field has some', () => {
    expect(mountField({ key: 'a', label: 'A', type: 'text', default: '', help: 'why' }, '').text()).toContain('why')
    expect(
      mountField({ key: 'a', label: 'A', type: 'text', default: '' }, '').find('.recipe-field-help').exists(),
    ).toBe(false)
  })

  it('coerces a stored value of the wrong shape for a number field', () => {
    const asString = mountField({ key: 'n', label: 'N', type: 'number', default: 5 }, '12' as unknown as FieldValue)
    expect(asString.findComponent({ name: 'InputNumber' }).props('modelValue')).toBe(12)

    const nonsense = mountField({ key: 'n', label: 'N', type: 'number', default: 5 }, 'abc' as unknown as FieldValue)
  ...
  it('shows an empty box for a non-string value on a text field', () => {
    const wrapper = mountField({ key: 't', label: 'T', type: 'text', default: '' }, 7 as unknown as FieldValue)
  ...
  it('prefers the field placeholder over the generic one', () => {
    ...
    const time = mountField({ key: 't', label: 'T', type: 'time', default: '', placeholder: '22:00' }, '')
  ...
  // InputText and InputMask are plain inputs with no `inputId` prop, while
  // InputNumber/Select/MultiSelect/ToggleSwitch all have one and ignore a
  // bare `id`. Getting that backwards leaves the <label for> pointing at
  // nothing — invisible in a component test that clicks the element
  // directly, and the reason a browser test could not open the camera
  // picker by its label's id.
  it.each([
    ['number' as const, {}],
    ['text' as const, {}],
    ['toggle' as const, {}],
    ['time' as const, {}],
    ['select' as const, { options: [{ label: 'A', value: 'a' }] }],
    ['multiselect' as const, { options: [{ label: 'A', value: 'a' }] }],
  ])('gives a %s field a label that points at a real element', (type, extra) => {
    const wrapper = mountField(
      { key: 'thing', label: 'Thing', type, default: type === 'multiselect' ? [] : '', ...extra },
      type === 'multiselect' ? [] : '',
    )
    const target = wrapper.find('label').attributes('for')
    expect(target).toBe('recipe-field-thing')
    expect(wrapper.find(`#${target}`).exists()).toBe(true)
  })

  it('tags the wrapper with the field key, for tests that must click the widget', () => {
    const wrapper = mountField({ key: 'cameras', label: 'Cameras', type: 'multiselect', default: [] }, [])
    expect(wrapper.find('.recipe-field-cameras').exists()).toBe(true)
  })
})
