...
describe('AdaptiveLearningCard', () => {
  ...
  it('shows "No feedback recorded yet" with zero total', async () => {
    vi.stubGlobal('fetch', vi.fn(() => Promise.resolve(jsonResponse({ total: 0, correct: 0, incorrect: 0, false_positive: 0, false_negative: 0 }))))
  ...
  it('shows accuracy percent and breakdown, colored green at >=80%', async () => {
    vi.stubGlobal('fetch', vi.fn(() => Promise.resolve(jsonResponse({ total: 10, correct: 9, incorrect: 1, false_positive: 1, false_negative: 0 }))))
    const wrapper = mount(AdaptiveLearningCard)
    await flushPromises()
  ...
  it('colors accuracy yellow between 50-79% and red under 50%', async () => {
    vi.stubGlobal('fetch', vi.fn(() => Promise.resolve(jsonResponse({ total: 10, correct: 6, incorrect: 4, false_positive: 2, false_negative: 2 }))))
    const wrapper = mount(AdaptiveLearningCard)
    await flushPromises()
    expect(wrapper.text()).toContain('60%')

    vi.stubGlobal('fetch', vi.fn(() => Promise.resolve(jsonResponse({ total: 10, correct: 2, incorrect: 8, false_positive: 4, false_negative: 4 }))))
  ...
  it('reload() re-fetches stats (exposed for the parent to call after events)', async () => {
    vi.stubGlobal('fetch', vi.fn(() => Promise.resolve(jsonResponse({ total: 1, correct: 1, incorrect: 0, false_positive: 0, false_negative: 0 }))))
    const wrapper = mount(AdaptiveLearningCard)
    await flushPromises()
