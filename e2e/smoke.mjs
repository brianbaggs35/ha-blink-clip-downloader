...
//
// TAB_CHECKS is also imported by e2e/ha_integration_smoke.mjs (the real
// Home Assistant Supervisor/ingress integration test) so both stay in sync
// as tabs are added/removed - only the tab list/text below needs updating
...
// Only run the script body when invoked directly (`node smoke.mjs ...`),
// not when TAB_CHECKS is imported by e2e/ha_integration_smoke.mjs - that
// script drives its own browser/page through a login+ingress flow first
// and reuses waitForOwnTabContent below directly instead.
